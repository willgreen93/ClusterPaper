...
//[[Rcpp::export]]
NumericMatrix ct_func2_cpp_thresh_peak(const NumericVector& a, const NumericVector& b, const NumericVector& c, const NumericVector& l, const NumericVector& thresh,
...
#pragma omp parallel for schedule(static,1)
  for(int tn=0;tn<ncores;tn++) {
    ...
    for(int k=l; k<m; k++){
      ...
      if(vk==1){
        double t1=(tsv/asv)-lag;
        //double vt=(csv-tsv-vli)/asv;
        for(int j=0;j<t_n;j++) { // 7 should be 0
          int t=(double) j;
            //if(csv-tsv < vli) cv = vli;
            //else{
            if(j <= t1+tstoch) cv = csv-tsv+asv*(lag+t-tstoch);
            //else if(j <= t1) cv = csv;
